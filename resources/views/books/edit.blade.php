                ...
                <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control" name="status" id="status">
                            <option value="available" {{ old('status', $book->status) == 'available' ? 'selected' : '' }}>ទំនេ</option>
                            <option value="borrowed" {{ old('status', $book->status) == 'borrowed' ? 'selected' : '' }}>មិនទំនេរ</option>
                            <option value="reserved" {{ old('status', $book->status) == 'reserved' ? 'selected' : '' }}>ខូចខាត</option>
                        </select>
                    </div>

                    <div class="form-group mb-3">
                        <label for="cover_image">រូបភាពក្រប</label>
                        <input type="file" name="cover_image" id="cover_image" class="form-control" onchange="previewImage(event)">
